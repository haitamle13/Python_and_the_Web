...
class Project:
    ...
    def set_(self):
        ...
        if self.count == len(self.que):
            messagebox.showinfo(
                "Result",
                "You have answered {0} out of {1} questions correctly".format(
...
course_menu.add_radiobutton(
    label="Python", value="Python", variable=s, command=obj.course
)
course_menu.add_radiobutton(
    label="C", value="C", variable=s, command=obj.course
)
...
b[0] = Button(
    ...
    command=lambda: obj.fun(v[0].get(), b[0]),
)
...
b[1] = Button(
    ...
    anchor="w",
    width=20,
    command=lambda: obj.fun(v[1].get(), b[1]),
)
...
b[2] = Button(
    ...
    anchor="w",
    width=20,
    command=lambda: obj.fun(v[2].get(), b[2]),
)
...
b[3] = Button(
    ...
    anchor="w",
    width=20,
    command=lambda: obj.fun(v[3].get(), b[3]),
)
